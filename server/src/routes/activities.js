...
router.put('/:id', async (req, res) => {
  try {
    const existing = await db.prepare('SELECT * FROM activities WHERE id = ?').get(req.params.id);
    ...
    res.json(await db.prepare('SELECT * FROM activities WHERE id = ?').get(req.params.id));
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    const result = await db.prepare('DELETE FROM activities WHERE id = ?').run(req.params.id);
