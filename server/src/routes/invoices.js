...
router.get('/', async (req, res) => {
  try {
    ...
    const total = (await db.prepare('SELECT COUNT(*) as total FROM invoices').get()).total;
    const totalRevenue = (await db.prepare('SELECT SUM(total) as total FROM invoices WHERE status = ?').get('paid'))?.total || 0;
    const outstanding = (await db.prepare('SELECT SUM(total) as total FROM invoices WHERE status IN (?, ?)').get('sent', 'overdue'))?.total || 0;
...
router.get('/:id', async (req, res) => {
  try {
    const invoice = await db.prepare(`SELECT i.*, c.first_name || ' ' || c.last_name as contact_name, c.email as contact_email,
      cmp.name as company_name FROM invoices i
      LEFT JOIN contacts c ON i.contact_id = c.id
...
router.post('/', async (req, res) => {
  try {
    ...
    await db.prepare(`INSERT INTO invoices (id, invoice_number, contact_id, company_id, status, issue_date, due_date, subtotal, tax_rate, tax_amount, discount, total, currency, notes)
      VALUES (?, ?, ?, ?, 'draft', ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ...
    if (items && items.length > 0) {
      ...
      for (const item of items) {
        await insertItem.run(uuidv4(), id, item.description, item.quantity || 1, item.unit_price || 0, item.total || 0);
      }
    }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await db.prepare(`UPDATE invoices SET ${updates.join(', ')} WHERE id = ?`).run(...params);

    if (req.body.items) {
      await db.prepare('DELETE FROM invoice_items WHERE invoice_id = ?').run(req.params.id);
      const insertItem = db.prepare('INSERT INTO invoice_items (id, invoice_id, description, quantity, unit_price, total) VALUES (?, ?, ?, ?, ?, ?)');
      for (const item of req.body.items) {
        await insertItem.run(uuidv4(), req.params.id, item.description, item.quantity || 1, item.unit_price || 0, item.total || 0);
      }
    }
...
router.delete('/:id', async (req, res) => {
  try {
    const result = await db.prepare('DELETE FROM invoices WHERE id = ?').run(req.params.id);
