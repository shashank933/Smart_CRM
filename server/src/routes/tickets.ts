...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const tickets = await db.prepare(query).all(...params) as (TicketRow & Record<string, unknown>)[];
    const total = ((await db.prepare('SELECT COUNT(*) as count FROM tickets').get()) as { count: number }).count || 0;
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    const ticket = await db.prepare(`SELECT t.*,
      c.first_name || ' ' || c.last_name as contact_name,
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    await db.prepare(`INSERT INTO tickets (id, ticket_number, subject, description, status, priority, contact_id, company_id, deal_id, assigned_to, category, source, tags)
      VALUES (?, ?, ?, ?, 'open', ?, ?, ?, ?, ?, ?, ?, ?)`)
    ...
    const ticket = await db.prepare('SELECT * FROM tickets WHERE id = ?').get(id) as TicketRow & Record<string, unknown> & Record<string, unknown>;
...
router.put('/:id', async (req: Request, res: Response) => {
  try {
    const existing = await db.prepare('SELECT * FROM tickets WHERE id = ?').get(req.params.id);
    ...
    if (updates.length > 0) {
      ...
      await db.prepare(`UPDATE tickets SET ${updates.join(', ')} WHERE id = ?`).run(...params);
    }

    const ticket = await db.prepare('SELECT * FROM tickets WHERE id = ?').get(req.params.id) as TicketRow & Record<string, unknown>;
...
router.delete('/:id', async (req: Request, res: Response) => {
  try {
    const result = await db.prepare('DELETE FROM tickets WHERE id = ?').run(req.params.id);
...
router.post('/:id/comments', async (req: Request, res: Response) => {
  try {
    ...
    await db.prepare(`INSERT INTO ticket_comments (id, ticket_id, content, author_type, is_internal)
      VALUES (?, ?, ?, ?, ?)`)
    ...
    const comment = await db.prepare('SELECT * FROM ticket_comments WHERE id = ?').get(commentId);
...
router.post('/:id/summarize', async (req: Request, res: Response) => {
  try {
    const ticket = await db.prepare(`SELECT t.*, c.first_name || ' ' || c.last_name as contact_name
...
router.post('/:id/suggest-reply', async (req: Request, res: Response) => {
  try {
    ...
    const comments = await db.prepare('SELECT * FROM ticket_comments WHERE ticket_id = ? ORDER BY created_at ASC LIMIT 10').all(req.params.id);
    const thread = comments.map((c: { author_type: string; content: string }) => `${c.author_type}: ${c.content}`).join('\n');
